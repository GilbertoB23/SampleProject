...
For detailed setup instructions, see [SETUP.md](SETUP.md) - a beginner-friendly guide for team members.
...
2. **Configure database credentials:**
   ...
   - Edit the `.env` file and update with your database credentials:
     ...
     ```
   - **Important**: The `.env` file is gitignored and will not be shared. Each team member should create their own `.env` file with their local database credentials.
...
- Check that the `.env` file exists and contains correct database credentials
- Ensure the database `task_management` exists
- Verify your `.env` file has the correct format (no spaces around the `=` sign)
